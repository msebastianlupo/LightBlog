...
function PostDelete(){
    ...
    return (
        <>
            <h1 className="tex-a-c">Eliminación de entrada de blog: {post.title}</h1>
            <div>
                <p className="mar-t-4 tex-a-c errcol tex-2">{errors}</p>
                <form className="chidis-f chidir-c chiinv mar-t-2 chimar-t-2 neu-1 pad-2 rad-1" onSubmit={deletePost}>
                    <p className="tex-2-2">Advertencia: esta operación no se puede deshacer</p>
                    <Submit text="Eliminar" styles="rad-0-5 pad-1-5 hovbac-d" />
                </form>
            </div>
        </>
    )
